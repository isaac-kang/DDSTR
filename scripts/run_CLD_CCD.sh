...
ERROR_DIR="${DDSTR_DATA_ROOT}/error_info/${MODEL}"
CLD_OUTPUT="${DDSTR_DATA_ROOT}/CLD/${MODEL}__${LLM_NAME}_a${ALPHA}"
CLD_CCD_OUTPUT="${DDSTR_DATA_ROOT}/CLD_CCD/${MODEL}__${LLM_NAME}_a${ALPHA}"

mkdir -p "${ERROR_DIR}"

if run_step 1; then
    echo "=== Step 1: Extract error info (${MODEL}) ==="
    _T0=$SECONDS
...
if run_step 5; then
    echo ""
    echo "=== Step 5: Train with denoised + decomposed data (${MODEL} CLD_CCD) ==="
    _T0=$SECONDS
